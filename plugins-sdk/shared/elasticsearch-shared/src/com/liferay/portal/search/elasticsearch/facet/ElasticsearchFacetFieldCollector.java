...
public class ElasticsearchFacetFieldCollector implements FacetCollector {
	...
	public List<TermCollector> getTermCollectors() {
		...
		Collections.sort(termCollectors, new Comparator<TermCollector>() {
			@Override
			public int compare(
				TermCollector termCollector1, TermCollector termCollector2) {

				if (termCollector1.getFrequency() <
						termCollector2.getFrequency()) {
	...
	protected void initialize(Range range) {
		...
		for (Range.Bucket bucket : range.getBuckets()) {
			...
			String key = StringUtil.replace(
				(String)bucket.getKey(), StringPool.DASH, _TO_STRING);
	...
	protected void initialize(Terms terms) {
		...
		for (Terms.Bucket bucket : terms.getBuckets()) {
			...
			_counts.put(term, (int)bucket.getDocCount());
		}
	}
